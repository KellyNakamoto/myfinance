...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeDefaultData(); // Инициализируем данные по умолчанию
    loadInitialData();
...
// Управление вкладками (убрана аналитика)
...
// ИСПРАВЛЕННАЯ ФУНКЦИЯ updateAllCalculations с защитой от NaN
function updateAllCalculations() {
    ...
    // Обновляем прогресс бары с проверками
    const todayProgressBar = document.getElementById('todayProgress');
    if (todayProgressBar) {
        const progressValue = isNaN(todayProgress) ? 0 : todayProgress;
        todayProgressBar.style.width = `${progressValue}%`;
        
        let progressClass = 'primary';
        if (progressValue > 100) progressClass = 'error';
        else if (progressValue > 80) progressClass = 'warning';
        
        todayProgressBar.className = `progress-fill ${progressClass}`;
    }
...
// Улучшенная функция updateElement с проверками
function updateElement(id, value) {
    try {
        const element = document.getElementById(id);
        if (element) {
            // Дополнительная проверка на случай если value содержит NaN
...
// Улучшенная функция formatCurrency с защитой от NaN
